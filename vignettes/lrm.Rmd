...
We'll use simulated data of 100 samples, 10 predictors to show the way of using lrm() and compare it with the results from glm().
```{r}
set.seed(1)
...
```{r}
# fit glm
glm.fit = glm(y ~ as.matrix(X), family = binomial, start=c(rep(0, 11)))
summary(glm.fit)
# fit lrm
lrm.fit = lrm(y, X)
```
...
```{r}
# fit glm
glm.fit = glm(y ~ -1 + as.matrix(X), family = binomial, start=c(rep(0, 10)))
summary(glm.fit)
# fit lrm
