...
```{r}
Xnew = sapply(1:10, function(i)rnorm(5, mean(X[,i]), sd(X[,i])))
...
We can also compare the runtime of lrm() nad glm()
```{r}
...
cbind(c("lrm()", "glm()"), compare_result[, 3])
```
We see the the lrm() performs slower than glm(), which is reasonable with the IRLS coding in R. The runtime would improve the commented Rcpp code in the src file. You can also check https://github.com/y1zhong/LogisticRegRcpp for more reference.
